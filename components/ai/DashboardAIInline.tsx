"use client"

// ==========================================================
// HisabDesk — Dashboard AI Inline Assistant
// ----------------------------------------------------------
// PURPOSE
//   Inline AI box specifically for Dashboard context
//
//   Difference vs Floating Assistant:
//     Floating → generic
//     Inline  → dashboard-aware (auto context injected)
//
//   Features:
//     ✓ auto financial context injection
//     ✓ short answers
//     ✓ cheap chat model
//     ✓ safeRunAI via /api/ai/page-assistant
//
//   Why this exists:
//     ✓ AI feels native to dashboard
//     ✓ quick “what should I improve?”
//     ✓ better than opening global chat
//
//   Uses:
//     useDashboardContext()
//     POST /api/ai/page-assistant
//
//   Usage:
//     <DashboardAIInline />
//
// ==========================================================

import { useState } from "react"
import { Card } from "@/components/ui/card"

import { useDashboardContext } from "@/lib/hooks/useDashboardContext"

// ==========================================================
// COMPONENT
// ==========================================================

export default function DashboardAIInline() {
  const { context, loading: ctxLoading } = useDashboardContext()

  const [message, setMessage] = useState("")
  const [reply, setReply] = useState("")
  const [loading, setLoading] = useState(false)

  // --------------------------------------------------------
  // ASK AI
  // --------------------------------------------------------

  async function ask() {
    if (!message.trim() || ctxLoading) return

    try {
      setLoading(true)
      setReply("")

      const res = await fetch("/api/ai/page-assistant", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          message,
          contextString: context, // compact injected context
        }),
      })

      const json = await res.json()

      setReply(json.reply || "")
      setMessage("")
    } finally {
      setLoading(false)
    }
  }

  // ========================================================
  // UI
  // ========================================================

  return (
    <Card className="p-4 space-y-3">
      <div className="font-semibold text-sm">
        Ask AI about your finances
      </div>

      {/* Reply */}
      {reply && (
        <div className="text-sm bg-muted p-2 rounded whitespace-pre-wrap">
          {reply}
        </div>
      )}

      {/* Input */}
      <textarea
        value={message}
        onChange={(e) => setMessage(e.target.value)}
        placeholder={
          ctxLoading
            ? "Preparing context…"
            : "How can I improve savings this month?"
        }
        className="w-full border rounded p-2 text-sm resize-none"
        rows={3}
        disabled={ctxLoading || loading}
      />

      {/* Button */}
      <button
        onClick={ask}
        disabled={ctxLoading || loading}
        className="w-full bg-primary text-white rounded py-2 text-sm"
      >
        {loading ? "Thinking…" : "Ask AI"}
      </button>
    </Card>
  )
}
