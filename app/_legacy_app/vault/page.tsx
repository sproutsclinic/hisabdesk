...
import VaultStats from "@/components/vaults/VaultStats"
...
export default function VaultPage() {
  ...
  const init = async () => {
    ...
    // ✅ BETA MODE — NO FORCED REDIRECT
    // Vault must open without login
    if (!user) {
      await loadItems()
      return
    }
          ...
          {filteredCategories.map((cat) => {
            ...
            return (
              <Card
                ...
                variant="interactive"
                className="p-5"
                onClick={() => window.location.href = `/vault/${cat.key}`}
              >
