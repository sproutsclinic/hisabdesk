...
/* =========================================================
   HisabDesk — Wealth Planner (ENTERPRISE FINAL)
   ---------------------------------------------------------
   ✓ strategy page (not daily entries)
   ✓ net worth tracking
   ✓ asset allocation
   ✓ growth trend
   ✓ goals
...
export default function WealthPlannerPage() {
  ...
  async function loadAI() {
    const res = await fetch("/api/ai/wealth-advice", {
      method: "POST",
    })
  ...
  if (loading)
    return <p className="p-6 text-sm">Loading wealth data...</p>

  if (!data)
    return <p className="p-6 text-sm">No data</p>
