...
✓ AIS income
✓ Books (transactions)
✓ Presumptive 44ADA/44AD logic
✓ Deductions detection
...
export async function generateTaxSuggestions(orgId: string) {
  ...
  if (unreconciled.length) {
    ...
    suggestions.push({
      ...
      message: `₹${format(amount)} income appears in AIS but not in books. Add entries to avoid notice.`,
      impact: amount,
      priority: "high",
    })
  }
  ...
  if (revenue <= 7500000) {
    ...
    suggestions.push({
      ...
      message: `You may declare ₹${format(
        presumptiveIncome
